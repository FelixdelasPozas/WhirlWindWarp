...
//--------------------------------------------------------------------
WhirlWindWarp::WhirlWindWarp(const int numPoints, const Utils::Configuration& config,
                             Utils::NumberGenerator* generator) :
...
void WhirlWindWarp::postUpdateState()
{
    ...
    for (int i = 0; i < fs; ++i) {
        ...
        if (m_state.enabled[i]) {
            ...
            m_state.var[i] =
                m_state.optimum[i] + (m_state.var[i] - m_state.optimum[i]) * 0.9995 + 0.001 * m_state.velocity[i];
        }
        ...
        if (!m_state.enabled[i] && (m_generator->get() > prob_on())) {
            ...
            if (m_state.enabled[i] && (m_generator->get() > 0.99) &&
                fabs(m_state.var[i] - m_state.optimum[i]) < 0.0005 &&
