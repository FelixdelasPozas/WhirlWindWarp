...
int APIENTRY wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nShowCmd)
{
    try {
        ...
        // int g_nCmdShow = nShowCmd;

        if (TIMERR_NOERROR != timeBeginPeriod(1)) {
            std::cerr << "Unable to set timer resolution." << std::endl;
        }

        std::atexit(exit_handler);
        std::set_terminate(exit_handler);
        ...
        for (;;) {
            switch (*cmdline) {
                ...
                case TEXT('P'):
                {
                    ...
                    HINSTANCE g_hParent = (HINSTANCE)(wcstoull(cmdline, nullptr, 10));
                    // g_sm = SAVER_SAVER;
                    return (g_hParent && nullptr != GetParent((HWND)g_hParent)) ? run() : EXIT_FAILURE;
                }
                ...
                case TEXT('C'):
                    MessageBox(nullptr, TEXT("ElectroPaint has no options."), TEXT("CLASSNAME"), MB_OK | MB_ICONINFORMATION);
                    return EXIT_SUCCESS;
